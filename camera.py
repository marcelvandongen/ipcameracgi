...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    async def async_handle_ptz(service):
        ...
        for camera in target_cameras:
            _LOGGER.debug("Moving '%s' camera: %s", movement, entity_ids)
...
class HassFoscamCamera(Camera):
    ...
    async def stream_source(self):
        ...
        if self._rtsp_port:
            return "http://{}:{}@{}:{}/videoStream.cgi".format(
                self._username,
                self._password,
    ...
    async def async_perform_ptz(self, movement, travel_time):
        ...
        _LOGGER.debug("movement function: '%s'", movement_function)

        ret, ret2 = await self.hass.async_add_executor_job(movement_function)

        if ret != 0:
            _LOGGER.error("Error moving %s '%s': %s", movement, self._name, ret)
            _LOGGER.error("Extra error message: '%s'", ret2)
            return
